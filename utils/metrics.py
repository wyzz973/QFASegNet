...
def psnr(im1, im2, peakval=1.):
    # 确保输入在GPU上
    assert im1.device.type == 'cuda' and im2.device.type == 'cuda', "Input images must be on GPU"

    # 检查输入图像的值范围
    assert (im1.max() <= 1.) and (im1.min() >= -1.), "im1 values must be between -1 and 1"
    assert (im2.max() <= 1.) and (im2.min() >= -1.), "im2 values must be between -1 and 1"
    ...
    return snr.item()  # .item() 转换为Python标量
...
def calc_multi_dice(result, gt, num_cls):
    ...
    for c in range(1, num_cls):
        _pred = (result == c).float()
        _label = (gt == c).float()

        intersection = torch.sum(_pred * _label)
        union = torch.sum(_pred) + torch.sum(_label)

        if union == 0:  # Check if both prediction and ground truth are empty
            dice_score = torch.tensor(0.0)  # Ensure dice_score is a tensor
        else:
            dice_score = (2. * intersection + 1e-6) / (union + 1e-6)

        dice_list[c - 1] = dice_score.item()  # Convert to Python float for storing
...
def calc_multi_asd(result, gt, num_cls):
    assert num_cls > 2

    # asd_list = [0.] * (num_cls - 1)
    asd_list = [np.nan] * (num_cls - 1)  # 使用 NaN 初始化列表
    for c in range(1, num_cls):
        _pred = result.copy()
        _pred[_pred != c] = 0
        ...
        _label = gt.copy()
        _label[_label != c] = 0
        if np.count_nonzero(_pred) == 0 or np.count_nonzero(_label) == 0:
            continue  # Skip the current class if either array is empty
        asd_list[c - 1] = asd(_pred, _label)
...
def dc(result, reference):
    ...
    Computes the Dice coefficient (also known as Soren-sen index) between the binary
    ...
    ----------
    result : array_like
        Input datas containing objects. Can be any type but will be converted
...
def volume_correlation(results, references):
    ...
    results : sequence of array_like
        Ordered list of input datas containing objects. Each array_like will be
